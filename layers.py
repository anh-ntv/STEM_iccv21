...
def batch_ema_acc(a, b, scope=None):
    with tf.name_scope(scope, 'basic_acc'):
        a = tf.argmax(a, 1)  # (batch_size,)
        b = tf.argmax(b, 1)  # (batch_size,)
        output = tf.cast(tf.equal(a, b), 'float32')  # (batch_size,)
    return output
...
def batch_teac_stud_avg_acc(y_trg_true, y_trg_logit, y_trg_teacher,  scope=None):
    with tf.name_scope(scope, 'average_acc'):
        ...
        y_pred_avg = (y_trg_prob + y_trg_teacher) / 2.0  # (bs, n_classes)

        y_trg_true = tf.argmax(y_trg_true, 1)
        y_pred_avg = tf.argmax(y_pred_avg, 1)
        output = tf.cast(tf.equal(y_trg_true, y_pred_avg), 'float32')  # (batch_size,)
    return output
...
def batch_teac_stud_ent_acc(y_trg_true, y_trg_logit, y_trg_teacher,  scope=None):
    with tf.name_scope(scope, 'entropy_acc'):
        y_trg_prob = tf.nn.softmax(y_trg_logit)
        # compute entropy
        y_trg_student_ent = -tf.reduce_sum(y_trg_prob * tf.log(y_trg_prob), axis=-1)  # (bs, 10) * (bs, 10)
        y_trg_teacher_ent = -tf.reduce_sum(y_trg_teacher * tf.log(y_trg_teacher), axis=-1)
        min_entropy = tf.argmin(tf.stack([y_trg_student_ent, y_trg_teacher_ent]), axis=0)  # indices (bs,)
        ...
        student_teacher_concat = tf.stack([y_trg_pred_sparse, y_trg_teacher_sparse], axis=1)  # (bs, 2)
        ...
        output = tf.cast(tf.equal(y_trg_true, y_pred_entropy_voting), 'float32')  # (batch_size,)
    return output